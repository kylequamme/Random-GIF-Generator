...
$(function(){
  ...
  function getGIF(tag){
    ...
    $.ajax('http://api.giphy.com/v1/gifs/random?api_key=dc6zaTOxFJmzC&tag=' + tag).success(function(giphy){
        ...
        $('main').append('<p><a href="' + giphy.data.image_url + '">' + giphy.data.image_url + '</a></p>');
      }
      )
      .fail(function(){
    ...
    function getStickers(tag){
      ...
      $.ajax('http://api.giphy.com/v1/stickers/random?api_key=dc6zaTOxFJmzC&tag=' + tag).success(function(giphy){
        ...
        $('.result').append('<img src="' + giphy.data.image_url + '" width="500px"/>');
        $('.result').append('<p><a href="' + giphy.data.image_url + '">' + giphy.data.image_url + '</a></p>');
      }
      )
        .fail(function(){
